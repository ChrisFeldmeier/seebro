...
var (
	...
	stateDir         = envOr("BRIDGE_STATE_DIR", filepath.Join(homeDir(), ".seebro"))
	headless         = os.Getenv("BRIDGE_HEADLESS") == "true"
	noRestore        = os.Getenv("BRIDGE_NO_RESTORE") == "true"
	profileDir       = envOr("BRIDGE_PROFILE", filepath.Join(homeDir(), ".seebro", "chrome-profile"))
...
func loadConfig() {
	...
	configPath := envOr("BRIDGE_CONFIG", filepath.Join(homeDir(), ".seebro", "config.json"))
...
func defaultConfig() Config {
	return Config{
		Port:        "9867",
		StateDir:    filepath.Join(homeDir(), ".seebro"),
		ProfileDir:  filepath.Join(homeDir(), ".seebro", "chrome-profile"),
		Headless:    true,
		NoRestore:   false,
		TimeoutSec:  15,
		NavigateSec: 30,
...
func handleConfigCommand() {
	if len(os.Args) < 3 {
		fmt.Println("Usage: seebro config <command>")
	...
	switch os.Args[2] {
	case "init":
		configPath := filepath.Join(homeDir(), ".seebro", "config.json")
